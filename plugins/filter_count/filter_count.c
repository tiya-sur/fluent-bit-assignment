...
static int counter_increment(struct count_ctx *ctx, const char *key)
{
    ...
    if (ctx->entry_count >= ctx->entry_cap) {
        ...
        struct counter_entry *tmp = flb_realloc(
            ctx->entries, sizeof(struct counter_entry) * (size_t)new_cap);
        if (!tmp) return -1;
        ctx->entries   = tmp;
                         ...
                         struct flb_config *config, void *data)
{
    ...
    if (!ctx->entries) { flb_free(ctx); return -1; }
    safe_strcpy(ctx->output_key, "count", sizeof(ctx->output_key));
    const char *val = flb_filter_get_property("output_key", f_ins);
    if (val) safe_strcpy(ctx->output_key, val, sizeof(ctx->output_key));
    flb_filter_set_context(f_ins, ctx);
    flb_plg_info(f_ins, "initialized. output_key=\"%s\" "
                 "counting by composite key (severity|brief|file|line)",
                 ctx->output_key);
    return 0;
}
...
#undef KIS
        ...
        make_alert_key(lv_b, msg_b, fi_b, line_val, alert_key, MAX_KEY_LEN);
